...
module Symbol=
    ...
    let tab = ref (new HashMultiMap<Identifier.id,entry> (0,HashIdentity.Structural))

    let initSymbolTable (size:int) =
       tab := new HashMultiMap<Identifier.id,entry> (size,HashIdentity.Structural)
    ...
    exception Exit
    let newEntry (id:Identifier.id) inf err =
      try
        if err then
