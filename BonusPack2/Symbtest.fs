...
open BonusPack
open BonusPack.Types
open BonusPack.Symbol
open BonusPack.Identifier
...
let pretty_id ppf id =
      fprintf ppf "%s" (Identifier.id_name id)
...
let printSymbolTable () =
  let rec walk ppf scp =
    if scp.sco_nesting <> 0 then begin
      fprintf ppf "scope: "
      let entry ppf e =
        fprintf ppf "%a" Identifier.pretty_id e.entry_id
        ...
        | ENTRY_function inf ->
            let param ppf e =
              match e.entry_info with
                ...
                | _ ->
                    fprintf ppf "<invalid>"
            let rec parameters ppf ps =
              match ps with
              | [p] ->
                  fprintf ppf "%a" param p
              | p :: ps ->
                  fprintf ppf "%a; %a" param p parameters ps
              | [] ->
                  ()
            fprintf ppf "(%a) : %a"
              parameters inf.function_paramlist
...
let main =
   ...
   let s1 = newVariable (id_make "s1") TYPE_byte true
   let s2 = newVariable (id_make "s2") TYPE_byte true
   let s3 = newVariable (id_make "s3") TYPE_byte true
   ...
   let i1 = newVariable (id_make "i1") TYPE_int true
   let i2 = newVariable (id_make "i2") TYPE_int true
   ignore i1; ignore i2;
   ...
   let p = newFunction (id_make "pr") true
   openScope ();
   ...
   let p1 = newParameter (id_make "p1") TYPE_int PASS_BY_VALUE p true
   let p2 = newParameter (id_make "p2") TYPE_int PASS_BY_VALUE p true
   let p3 = newParameter (id_make "p3") TYPE_byte PASS_BY_REFERENCE p true
   ...
   let b1 = newVariable (id_make "b1") TYPE_byte true
   ignore b1;
   ...
   let s1 = lookupEntry (id_make "s1") LOOKUP_ALL_SCOPES true
   let i2 = lookupEntry (id_make "i2") LOOKUP_ALL_SCOPES true
   let i1 = lookupEntry (id_make "i1") LOOKUP_ALL_SCOPES true
   ignore s1; ignore i2; ignore i1;

   (* SOURCE : (* make 2 new temporaries *) *)

   let t1 = newTemporary TYPE_int
   let t2 = newTemporary TYPE_byte
   ignore t1; ignore t2;
   ...
   let p = newFunction (id_make "f") true
   openScope ();
   ...
   let x = newParameter (id_make "x") TYPE_int PASS_BY_VALUE p true
   let y = newParameter (id_make "y") TYPE_byte PASS_BY_REFERENCE p true
